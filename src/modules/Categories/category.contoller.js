...
export const addCategory = async (req, res, next) => {
  ...
  if (isNameExists)
    return next(new Error("name is oready exists", { cause: 401 }));
  //2)create slug
//   console.log(name);
  const slug = slugify(name,'-');
  //3)upload image to cloudinary
  if (!req.file) return next(new Error("enter image data", { cause: 401 }));
  ...
  //rolleBack
  req.folder = `${process.env.MAIN_FOLDER_UPLOAD}/category/${folderId}`;
  ...
  req.savedDocus = { model: Category, id: category._id };
  res.status(200).json({
...
export const updateCateg = async (req, res, next) => {
  ...
  if (name) {
    if (oldCateg.name === name)
      return next(
        new Error("please enter defront category name", { cause: 400 })
      );
  }
...
export const getAllCateg = async (req, res, next) => {
  ...
  if (!categorys)
    return next(new Error("categorys not  found", { cause: 404 }));
  res.states(200).json({
    msg: " success",
    data: categorys,
  });
...
export const deleteCateg = async (req, res, next) => {
  ...
  await cloudinaryConnection().api.delete_resources_by_prefix(
    `${process.env.MAIN_FOLDER_UPLOAD}/category/${category.folderId}`
  );
...
export const getCategoryById = async (req, res, next) =>{
  ...
  const {categId}=req.params
  const category=await Category.findById(categId)
  if(!category)return next(new Error ("category not found use anther id",{cause:404}))
...
export const getAllCategors = async (req, res, next) =>{

  const {page,size,sort,...search}=req.query
  const featchers =new ApiFeatcher(req.query,Category.find()).pagination(page,size).sort(sort).search(search)
  const categories=await featchers.mongooseQuery
